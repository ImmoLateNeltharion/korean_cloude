...
export function addApprovedWordManual(rawWord: string, rawCount = 1) {
  const db = getDB();
  ...
  const existing = db
    .prepare(
      "SELECT id, count FROM words WHERE word = ? AND status = 'approved' ORDER BY id DESC LIMIT 1"
    )
    .get(word) as { id: number; count: number } | undefined;

  if (existing) {
    db.prepare("UPDATE words SET count = count + ? WHERE id = ?").run(count, existing.id);
    return { word, count: existing.count + count, updated: true };
  }

  db.prepare(
    "INSERT INTO words (word, count, source, status, moderated_at) VALUES (?, ?, 'manual', 'approved', datetime('now'))"
  ).run(word, count);
  return { word, count, updated: false };
}
